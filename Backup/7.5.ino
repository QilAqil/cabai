...
// ════════════════════════════════════════════════════════════════
//  ANTI-INTERFERENSI SOIL ↔ pH  (galvanic coupling)
//  Sensor soil moisture resistif mengalirkan arus kecil ke medium.
//  Jika elektroda pH berada dalam wadah yang sama, arus bocor
//  membuat ADC pH spike secara palsu.
//
//  Solusi perangkat lunak:
//  1. Baca soil DULU dengan multi-sample (kurangi noise)
//  2. Tunggu SOIL_SETTLE_MS → arus galvanik di medium hilang
//  3. BARU aktifkan DMS dan baca pH
//  4. pH cross-guard: tolak jika soil >85% DAN pH lompat >1.5 unit
// ════════════════════════════════════════════════════════════════
const unsigned long SOIL_SETTLE_MS   = 1500UL;  // jeda setelah soil sebelum DMS pH aktif
const int           SOIL_SAMPLES     = 20;       // jumlah sampel rata-rata soil
...
//  1. Celupkan elektroda ke buffer pH 4.01 → catat ADC dari Serial (pHADC:)
//  2. Celupkan ke buffer pH 6.86           → catat ADC
//  3. Celupkan ke buffer pH 9.18           → catat ADC
//  4. Update ketiga nilai ADC di bawah
//
//  Kalibrasi saat ini (diukur 04-Jul-2026):
//    ADC 1062 → pH 4.01  (buffer asam)
//    ADC  752 → pH 6.86  (dikoreksi dari data aktual: ADC 752 = pH 6.9 alat manual)
//    ADC  560 → pH 9.18  (estimasi, WAJIB diukur ulang dengan buffer 9.18)
// ════════════════════════════════════════════════════════════════
struct PhCalPoint { int adc; float ph; };
const PhCalPoint PH_CAL[3] = {
  { 1062, 4.01f },   // ← ukur ulang dengan buffer pH 4.01
  {  752, 6.86f },   // ← dikoreksi: ADC aktual saat pH 6.9
  {  560, 9.18f },   // ← estimasi proporsional, ukur ulang dengan buffer pH 9.18
};

const int   PH_SAMPLES        = 150;
const int   PH_TRIM_PCT       = 25;
const int   PH_MA_SIZE        = 10;      // dikurangi 20→10: lebih responsif, buffer lama tidak menarik nilai
const int   PH_MICRO_SIZE     = 5;
const float PH_EMA_ALPHA      = 0.15f;   // dinaikkan 0.05→0.15: ikuti perubahan nyata lebih cepat
const float PH_NOISE_GATE     = 0.80f;
const float PH_RATE_LIMIT     = 0.02f;   // dinaikkan 0.005→0.02: tidak terlalu lambat mengejar nilai
...
const int   PH_WARMUP_N       = 2;      // siklus warmup sebelum output
                                        // (dikurangi 5→2: semua buffer diisi penuh saat warmup)
...
float bacaPH() {
  ...
  //  Selama warmup, isi SEMUA buffer (micro + MA + EMA) sekaligus agar
  //  saat warmup selesai output langsung stabil — tidak perlu "pemanasan" ekstra.
  if (phWarmupCount < PH_WARMUP_N) {
    phWarmupCount++;
    Serial.printf("[pH] Warmup %d/%d → ADC=%d phRaw=%.3f (mengisi buffer)\n",
      phWarmupCount, PH_WARMUP_N, adcFinal, phRaw);

    // Isi semua slot buffer dengan nilai siklus ini
    ...
    // Pra-kondisi EMA, output, dan last agar tidak lompat dari 0
    phEMA    = phComp;
    phOutput = phComp;
    phLast   = phComp;

    return 0.0f;  // belum output — tunggu warmup selesai
  }
...
//  ADC 12-bit → rata-rata SOIL_SAMPLES sampel → petakan ke 0–100%
//
//  Anti-interferensi galvanic coupling dengan elektroda pH:
//  • Ambil rata-rata SOIL_SAMPLES (kurangi noise dari crosstalk)
//  • Setelah selesai, delay SOIL_SETTLE_MS sambil proses MQTT
//    agar arus sisa di medium hilang sebelum DMS pH diaktifkan
// ════════════════════════════════════════════════════════════════
float bacaSoil() {
  analogReadResolution(12);

  // Rata-rata SOIL_SAMPLES sampel
  long sumRaw = 0;
  ...
  // ── Settling delay anti-interferensi ────────────────────────
  // Beri waktu arus galvanik dari probe soil di medium hilang
  // sebelum elektroda pH dieksitasi. Tanpa ini ADC pH akan spike
  // saat kedua sensor berada dalam wadah yang sama.
  Serial.printf("[Soil] ADC=%d → %.1f%% | Settling %lums...\n",
    raw, result, SOIL_SETTLE_MS);
  unsigned long t0 = millis();
  while (millis() - t0 < SOIL_SETTLE_MS) {
    mqttClient.loop();
    delay(100);
  }
...
void terapkanAktuator(bool kipas, bool pompa_air, bool pompa_ph) {
  ...
  // — Logika Pompa Air: nyala 5 detik → mati → tunggu 30 detik → cek lagi
  unsigned long now = millis();
  if (pompa_air) {
    // Fuzzy minta pompa ON — cek apakah boleh nyala
    if (!pompaAirSedangNyala) {
      // Cek apakah jeda 30 detik setelah mati sudah lewat
    ...
    // Jika sedang nyala, cek apakah 5 detik sudah habis
  ...
  // — Logika Pompa pH: nyala 5 detik → mati → tunggu 2 jam → cek lagi
  if (pompa_ph) {
...
void loop() {
  ...
  if (now - lastBacaMillis >= INTERVAL_BACA) {
    ...
    // 2. Soil Moisture — dibaca DULU sebelum pH
    //    bacaSoil() sudah menyertakan SOIL_SETTLE_MS delay internal
    //    agar arus galvanik dari probe soil di medium hilang sebelum DMS pH aktif
    g_soil = bacaSoil();

    // 3. pH + DMS — dibaca SETELAH settling selesai
    float phBaru = bacaPH();

    // ── pH cross-contamination guard ──────────────────────────
    // Jika soil sangat basah (>85%) dan pH tiba-tiba lompat >1.5
    // kemungkinan besar masih ada sisa interferensi galvanik.
    // Tolak pembacaan dan pertahankan nilai g_pH sebelumnya.
    const float PH_CROSS_GUARD_DELTA = 1.5f;
    if (g_soil > 85.0f && g_pH > 0.0f &&
        fabsf(phBaru - g_pH) > PH_CROSS_GUARD_DELTA) {
      Serial.printf("[pH] Cross-guard: soil=%.1f%% delta=%.3f > %.2f, pakai pH lama=%.2f\n",
        g_soil, fabsf(phBaru - g_pH), PH_CROSS_GUARD_DELTA, g_pH);
    } else {
      g_pH = phBaru;
    }
