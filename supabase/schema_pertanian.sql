...
-- ESP32 POST: fuzzy_suhu, fuzzy_soil, fuzzy_ph, relay_kipas (0/1), relay_air, relay_ph
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS public.pertanian (
  ...
  -- relay_kipas: relay kipas (output fuzzy_suhu).
  relay_kipas    REAL NOT NULL DEFAULT 0,
  relay_air       REAL NOT NULL DEFAULT 0,
  relay_ph   REAL NOT NULL DEFAULT 0
);
...
ALTER TABLE public.pertanian ADD COLUMN IF NOT EXISTS relay_kipas REAL NOT NULL DEFAULT 0;
ALTER TABLE public.pertanian ADD COLUMN IF NOT EXISTS relay_ph REAL NOT NULL DEFAULT 0;

COMMENT ON TABLE public.pertanian IS 'Log sensor & kontrol IoT pertanian (MQTT + Supabase)';
COMMENT ON COLUMN public.pertanian.fuzzy_suhu IS 'Skor fuzzy suhu → relay kipas (0–1)';
...
COMMENT ON COLUMN public.pertanian.relay_kipas IS '0/1 relay kipas ON';
COMMENT ON COLUMN public.pertanian.relay_ph IS '0/1 relay koreksi pH';
