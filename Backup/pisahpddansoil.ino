...
//  Sensor soil moisture resistif mengalirkan arus melalui probe.
//  Jika elektroda pH berada dalam wadah yang sama, arus bocor
//  menaikkan ADC pH secara palsu (galvanic coupling).
//  Solusi: baca soil DULU, tunggu SOIL_SETTLE_MS (arus hilang),
//  BARU aktifkan DMS dan baca pH.
// ════════════════════════════════════════════════════════════════
const unsigned long SOIL_SETTLE_MS  = 1500UL;   // jeda setelah soil sebelum DMS pH aktif
const int           SOIL_SAMPLES    = 20;        // rata-rata beberapa sampel soil (anti-crosstalk)
...
//
//  Anti-interferensi:
//  • Ambil SOIL_SAMPLES sampel dengan jeda 5ms (kurangi noise spike)
//  • Setelah selesai, delay SOIL_SETTLE_MS agar arus sisa di medium
//    hilang sebelum elektroda pH diaktifkan
//  • Pin soil (GPIO35) adalah input-only ADC → tidak bisa dimatikan
//    secara aktif, tapi arus ekstra dari sensor kapasitif/resistif
//    sudah turun setelah pin tidak di-drive
// ════════════════════════════════════════════════════════════════
float bacaSoil() {
  analogReadResolution(12);

  // Rata-rata SOIL_SAMPLES sampel untuk kurangi noise
  long sumRaw = 0;
  ...
  // ── Jeda anti-interferensi ──────────────────────────────────
  // Setelah sampling soil selesai, tunggu agar arus galvanik dari
  // probe soil di medium (tanah/air) hilang sebelum elektroda pH
  // dieksitasi oleh DMS. Tanpa jeda ini, ADC pH akan spike saat
  // kedua sensor dalam wadah yang sama.
  Serial.printf("[Soil] ADC=%d → %.1f%% | Settling %lums...\n",
    raw, result, SOIL_SETTLE_MS);
  unsigned long settleStart = millis();
  while (millis() - settleStart < SOIL_SETTLE_MS) {
    mqttClient.loop();   // tetap proses MQTT selama settling
    delay(100);
  }
...
void loop() {
  ...
  if (now - lastBacaMillis >= INTERVAL_BACA) {
    ...
    //    agar arus galvanik dari probe soil hilang sebelum pH dieksitasi
    g_soil = bacaSoil();

    // 3. pH + DMS — dibaca SETELAH soil settling selesai
    float phBaru = bacaPH();

    // ── pH cross-contamination guard ──────────────────────────
    // Jika soil sangat basah (>85%) dan pH spike ekstrem jauh dari
    // nilai terakhir, kemungkinan besar masih ada interferensi sisa.
    // Tolak pembacaan — pertahankan g_pH lama.
    const float PH_SOIL_GUARD_DELTA = 1.5f;   // max perubahan pH saat soil >85%
    bool phDitolak = false;
    if (g_soil > 85.0f && g_pH > 0.0f &&
        fabsf(phBaru - g_pH) > PH_SOIL_GUARD_DELTA) {
      Serial.printf("[pH] Cross-guard: soil=%.1f%% delta=%.3f > %.3f, pakai pH lama=%.2f\n",
        g_soil, fabsf(phBaru - g_pH), PH_SOIL_GUARD_DELTA, g_pH);
      phDitolak = true;
    }
    if (!phDitolak) g_pH = phBaru;
