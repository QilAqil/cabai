...
// Flag: state relay berubah → kirim Supabase segera (tidak tunggu 1 menit)
volatile bool flagRelayBerubah = false;
...
float bacaSoil() {
  ...
  if (variansi > SOIL_VARIANSI_MAX) {
    Serial.printf("[Soil] Variansi=%d → tidak stabil, reset 0\n", variansi);
    soilKfEst  = 0.0f;
    soilKfP    = 1.0f;
    soilWarmup = true;
...
//
// DUA JALUR TERPISAH:
//   AUTO   : reqPompaAir / reqPompaPH   → nyala 10 detik, tunduk jeda
...
void tickPompa() {
  ...
  if (manualPompaAir) {
    ...
    if (!pompaAirNyala) {
      ...
      Serial.println("[PompaAir] ON (MANUAL)");
    }
    cdAir = -1;
    ...
    if (reqPompaAir) {
      if (pompaAirOffTime == 0 || now - pompaAirOffTime >= POMPA_AIR_JEDA) {
        ...
        pompaAirOnTime = now;
        flagRelayBerubah = true;
        Serial.println("[PompaAir] ON (AUTO) — nyala 10 detik");
      } else {
    ...
    // Sedang nyala AUTO — cek durasi 10 detik
    cdAir = -1;
  ...
  if (manualPompaPH) {
    ...
    if (!pompaPHNyala) {
      ...
      pompaPHNyala = true;
      flagRelayBerubah = true;
      Serial.println("[PompaPH] ON (MANUAL)");
    }
    cdPH = -1;
    ...
    if (reqPompaPH) {
      if (pompaPHOffTime == 0 || now - pompaPHOffTime >= POMPA_PH_JEDA) {
        ...
        pompaPHOnTime = now;
        flagRelayBerubah = true;
        Serial.println("[PompaPH] ON (AUTO) — nyala 10 detik");
      } else {
  ...
  // Update state relay pompa ke shared data setiap tick
  // Ini memastikan snapshot yang dibaca Supabase selalu akurat
  if (xSemaphoreTake(xMutex, pdMS_TO_TICKS(5)) == pdTRUE) {
    sd.relay_pompa_air = pompaAirNyala;
    sd.relay_pompa_ph  = pompaPHNyala;
...
void taskKomunikasi(void* param) {
  ...
  for (;;) {
    ...
    // Kirim Supabase segera jika ada perubahan relay
    if (flagRelayBerubah) {
      flagRelayBerubah = false;
      insertSupabase();
      lastDB = xTaskGetTickCount(); // reset timer agar tidak double-kirim
    }

    // Kirim Supabase tiap INTERVAL_DB_MS (data periodik normal)
